...
# Solution exercice 1
if st.checkbox("Voir la solution",key="code1"):
    st.code("""
    st.title("Afficher le Dataset")
df=pd.read_csv("Dataset_exercice.csv")
st.dataframe(df)""")
...
# solution filtre année
if st.checkbox("Voir la solution",key="code2"):
    st.code("""
    filtre_annee=sorted(df["Annee_voiture"].unique())
annee=st.selectbox("Filtre années",filtre_annee)
st.dataframe(df.loc[df["Annee_voiture"]==annee])""")
...
if st.checkbox("Voir la solution",key="code3"):
    st.code("""
    df_ex3=df["marque_voiture"].value_counts()
st.bar_chart(df_ex3)""")
...
if st.checkbox("Voir la solution",key="code4"):
    st.code("""
    df_ex4=df["Annee_voiture"].value_counts()
st.line_chart(df_ex4)""")
...
******.bar_chart(df_ex3)
""")
if st.checkbox("Voir la solution",key="code5"):
    st.code("""
    marque_voiture=df["marque_voiture"].unique()
annee=df["Annee_voiture"].unique()
col1, col2=st.columns(2)
col1.line_chart(df_ex4)
col2.bar_chart(df_ex3)""")
...
********(carte)
""")
if st.checkbox("Voir la solution",key="code6"):
    st.code("""
    carte=df[["latitude","longitude"]]
st.map(carte)""")
...
if st.checkbox("Voir la solution",key="code7"):
    st.code("""
   st.header("Prenez vous en photo puis envoyer là sur Slack")
picture = st.camera_input("camera")
if picture:
